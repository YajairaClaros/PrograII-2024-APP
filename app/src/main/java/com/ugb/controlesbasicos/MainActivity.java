...
public class MainActivity extends AppCompatActivity {
    ...
    String UrlCompletaFotoFirestrore;
    String miToken= "";
    ...
    private void subirFotoFirestore(){
        ...
        tareaSubir.addOnFailureListener(e->{
            mostrarMsg("Error al subir la foro a firebase: "+ e.getMessage());
        });
        tareaSubir.addOnSuccessListener(instantanea->{
            ...
            Task<Uri> descargarUri = tareaSubir.continueWithTask(tarea->reference.getDownloadUrl()).addOnCompleteListener(tarea->{
                if(tarea.isSuccessful() ){
                    UrlCompletaFotoFirestrore = tarea.getResult().toString();
                    guardarAmigo();
                }else{
    ...
    private void guardarAmigo(){
        try {
            ...
            if( miToken!="" && miToken!=null ){
                amigos amigo = new amigos(idAmigo,nombre,direccion,tel,email,dui,urlCompletaFoto,UrlCompletaFotoFirestrore,miToken);
                if(key!=null){
